...
snake_body::snake_body(terrain& habitat,
...
terrain& snake_body::get_terrain() const
{
    return this->habitat;
}

std::vector<dynamics> snake_body::get_trail() const
{
    return {std::cbegin(this->trail), std::cend(this->trail)};
}

dynamics snake_body::get_trail_head() const
{
    return this->current_dynamics;
}

canonical_direction snake_body::get_direction() const
{
    return get_footprint_direction(this->current_dynamics.step);
}

bool snake_body::is_position_in_tail(util::value_ref<position> pos) const
{
...
int snake_body::get_length() const
{
    return static_cast<int>(this->trail.size());
}

void snake_body::advance()
{
...
void snake_body::grow(int const size)
{
    this->cells_to_grow += size;
}

void snake_body::shrink(int const size)
{
    this->cells_to_shrink += size;
}

void snake_body::turn_left()
{    
...
boost::signals2::connection snake_body::register_movement_handler(
    movement_event_handler handler) const
{
    return this->on_movement.connect(handler);
}

void snake_body::grow_or_cut_last_tail_part()
{
